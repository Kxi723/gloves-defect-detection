...
def ellipsize(text: str, font: tkfont.Font, available: int) -> str:
...
class Button(tk.Label):
    VARIANTS = {"primary": (theme.ACCENT, "#ffffff", None, theme.ACCENT_DARK), "secondary": (theme.CARD, theme.INK, theme.LINE, theme.HOVER), "ghost": (None, theme.INK_SOFT, None, theme.MUTED_SOFT),}

    def __init__(self, parent, text: str, command: Optional[Callable] = None, variant: str = "secondary", parent_bg: str = theme.CARD, pad: int = 13, height: int = 30, stretch: bool = False, size: int = 9):
        ...
        super().__init__(parent, text=text, font=self._font, fg=fg, bg=parent_bg, bd=0, highlightthickness=0, compound="center", cursor="hand2")
    ...
    def _paint(self, hover: bool) -> None:
        ...
        self._surface = theme.surface(self._width, self._height, self._radius, fill, self._outline)
    ...
    def set_enabled(self, enabled: bool) -> None:
        ...
        self.configure(cursor="hand2" if enabled else "arrow", fg=self._fg if enabled else theme.INK_FAINT)
        if enabled:
            self._paint(hover=False)
        else:
            self._surface = theme.surface(self._width, self._height, self._radius, theme.MUTED_SOFT, None)
            self.configure(image=self._surface)
...
class Pill(tk.Label):
    def __init__(self, parent, text: str, fg: str, bg: str, parent_bg: str = theme.CARD, size: int = 8):
        ...
        super().__init__(parent, text=text, image=self._surface, font=font, fg=fg, bg=parent_bg, bd=0, highlightthickness=0, compound="center")
...
class ClipLabel(tk.Frame):
    def __init__(self, parent, text: str = "", font: Optional[tuple] = None, fg: str = theme.INK, bg: str = theme.CARD):
        self._font = tkfont.Font(font=font or theme.font(10))
        super().__init__(parent, bg=bg, width=px(40), height=self._font.metrics("linespace") + px(2))
        self.pack_propagate(False)
        self._label = tk.Label(self, text=text, font=self._font, fg=fg, bg=bg, anchor="w", bd=0, highlightthickness=0)
    ...
    def _refit(self) -> None:
        ...
        self._label.configure(text=ellipsize(self._full, self._font, available))
...
class Card(tk.Frame):
    ...
    def add_header(self, step: str, title: str) -> tk.Frame:
        ...
        badge = tk.Label(head, text=step, image=badge_surface, font=badge_font, fg=theme.ACCENT, bg=theme.CARD, compound="center", bd=0)
        badge.image = badge_surface
        badge.pack(side="left")
        tk.Label(head, text=title.upper(), font=theme.font(9, bold=True), fg=theme.INK, bg=theme.CARD).pack(side="left", padx=(px(8), 0))
...
class ScrollFrame(tk.Frame):
    def __init__(self, parent, bg: str = theme.CARD, padding: int = 0):
        ...
        self.canvas = tk.Canvas(self, bg=bg, bd=0, highlightthickness=0, width=1, height=1)
        self.canvas.pack(side="left", fill="both", expand=True)
        self._last_width = -1
        self.bar = tk.Canvas(self, bg=bg, width=px(8), bd=0, highlightthickness=0)
        self.bar.pack(side="right", fill="y")
        self._thumb = self.bar.create_rectangle(0, 0, 0, 0, fill=theme.SCROLL_THUMB, outline="")
    ...
    def _on_canvas(self, event) -> None:
        if event.width != self._last_width:
            ...
            self.canvas.itemconfigure(self._window, width=event.width - self._padding)
        self._draw_bar()
...
class DefectRow(tk.Frame):
    def __init__(self, parent, index: int, label: str, pending: bool, on_click: Callable[[int], None]):
        ...
        self._text = tk.Label(self, text=label, font=theme.font(10), fg=fg, bg=theme.CARD, anchor="w", bd=0)
    ...
    def set_selected(self, selected: bool) -> None:
        ...
        if not self.pending:
            self._text.configure(fg=theme.ACCENT if selected else theme.INK, font=theme.font(10, strong=selected))


class PhotoTile(tk.Frame):
    def __init__(self, parent, index: int, name: str, box: int, on_click: Callable[[int, object], None]):
        ...
        self._thumb = tk.Label(self, image=self._normal, bg=theme.CARD, bd=0, highlightthickness=0, cursor="hand2")
        self._thumb.pack()
        self._caption = ClipLabel(self, name, font=theme.font(8), fg=theme.INK_SOFT, bg=theme.CARD)
    ...
    def set_image(self, image_bgr: np.ndarray) -> None:
        ...
        self._chosen = theme.photo_thumb(image_bgr, self.box, radius, ring=theme.ACCENT, check=True)
        self._thumb.configure(image=self._chosen if self._selected else self._normal)
...
class ResultList(tk.Frame):
    ...
    def __init__(self, parent, on_select: Callable[[int], None], row_height: int = 52, on_activate: Optional[Callable[[int], None]] = None):
        super().__init__(parent, bg=theme.CARD)
        self._on_select = on_select
        self._on_activate = on_activate
        self._row_h = px(row_height)
        self.canvas = tk.Canvas(self, bg=theme.CARD, bd=0, highlightthickness=0, width=1, height=1)
        self.canvas.pack(side="left", fill="both", expand=True)
        self.bar = tk.Canvas(self, bg=theme.CARD, width=px(8), bd=0, highlightthickness=0)
        self.bar.pack(side="right", fill="y")
        self._bar_thumb = self.bar.create_rectangle(0, 0, 0, 0, fill=theme.SCROLL_THUMB, outline="")
    ...
    def clear(self, placeholder: str = "") -> None:
        ...
        if placeholder:
            self._placeholder = self.canvas.create_text(0, px(28), text=placeholder, font=theme.font(9), fill=theme.INK_FAINT, anchor="n")
    ...
    def append(self, name: str, status: str, score: float, evidence: str,
        ...
        pill_h = px(19)
        row = {"name": name,  "evidence": evidence.replace("\n", " "), "thumb": (theme.photo_thumb(thumb_bgr, side, px(6)) if thumb_bgr is not None else theme.placeholder_tile(side, px(6))), "pill_image": theme.surface(pill_w, pill_h, pill_h // 2, bg), "pill_w": pill_w,}
        row["bg"] = canvas.create_rectangle(0, top, 1, top + self._row_h, fill=theme.CARD, outline="")
        row["bar"] = canvas.create_rectangle(0, top, px(3), top + self._row_h, fill=theme.CARD, outline="")
        row["image"] = canvas.create_image(px(10), top + self._row_h // 2, image=row["thumb"], anchor="w")
        text_x = px(10) + side + px(10)
        row["title"] = canvas.create_text(text_x, top + px(16), text=name, font=self._name_font, fill=theme.INK, anchor="w")
        row["detail"] = canvas.create_text(text_x, top + px(34), text=row["evidence"], font=self._evidence_font, fill=theme.INK_SOFT, anchor="w")
        row["pill_bg"] = canvas.create_image(0, top + px(18), image=row["pill_image"], anchor="e")
        row["pill_text"] = canvas.create_text(0, top + px(18), text=status, font=self._pill_font, fill=fg, anchor="e")
        row["score"] = canvas.create_text(0, top + px(38), text=f"score {score:.2f}" if score else "", font=self._score_font, fill=theme.INK_FAINT, anchor="e")
        row["line"] = canvas.create_line(0, top + self._row_h, 1, top + self._row_h, fill=theme.LINE_SOFT)
    ...
    def _place_placeholder(self) -> None:
        if self._placeholder is not None:
            self.canvas.coords(self._placeholder, max(self._width, 2) // 2, px(28))
    ...
    def _layout_row(self, index: int) -> None:
        ...
        canvas.coords(row["line"], 0, top + self._row_h, width, top + self._row_h)
        right = width - px(12)
        ...
        canvas.itemconfigure(row["title"], text=ellipsize(row["name"], self._name_font, available))
        canvas.itemconfigure(row["detail"], text=ellipsize(row["evidence"], self._evidence_font, available))

    def _paint_row(self, index: int) -> None:
        selected = index == self._selected
        fill = theme.SELECTED if selected else (theme.HOVER if index == self._hover else theme.CARD)
        self.canvas.itemconfigure(self._rows[index]["bg"], fill=fill)
        self.canvas.itemconfigure(self._rows[index]["bar"], fill=theme.ACCENT if selected else fill)
    ...
    def _on_double_click(self, event) -> None:
        index = self._index_at(event)
        if index >= 0 and self._on_activate is not None:
